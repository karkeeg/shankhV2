...
function mcqStepsFromLesson(mcqActivity: any, lessonName: string): any[] {
  const questions = mcqActivity.questions ?? [];
  return questions.map((q: any, qi: number) => ({
    ...
    lessonName,
    activityData: {
      instructions: mcqActivity.instructions,
      context: mcqActivity.context,
    },
  }));
}

function canvasStepFromLesson(canvasActivity: any, lessonName: string): any {
  const edges = (canvasActivity.solutionEdges ?? []).map((e: any) => ({
  ...
  return {
    ...
    activityData: {
      ...
      tokens: canvasActivity.tokens ?? [],
      paletteItems: canvasActivity.paletteItems ?? [],
      solutionSnapshot: edges.length > 0 ? { edges, nodePositions: [] } : null,
    },
  };
}

function quantusStepFromLesson(quantusActivity: any, lessonName: string): any {
  const cols = [...(quantusActivity.columns ?? [])].sort(
    (a: any, b: any) => a.colIndex - b.colIndex
  );
  const cells = quantusActivity.quantusCells ?? [];
  const rowIndices = [
    ...new Set<number>(cells.map((c: any) => c.rowIndex as number)),
  ].sort((a, b) => a - b);
  ...
  return {
    id: quantusActivity.id,
    activityId: quantusActivity.id,
    stepType: "quantus",
    lessonName,
    activityData: {
      instructions: quantusActivity.instructions,
      context: quantusActivity.context,
      gridRows,
      gridCols,
      gridValues,
      correctAnswers,
...
export default function TestPreviewPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const uniqueLessonIds = [...new Set(allItems.map((i: any) => i.lessonId as string))];
      ...
      lessonResults.forEach((l) => {
        if (l?.id) lessonMap[l.id] = l;
      });

      const builtSteps: any[] = [];
      for (const item of allItems) {
        const lesson = lessonMap[item.lessonId];
        if (!lesson) continue;
        const lName = lesson.name || item.lessonName || "Lesson";

        if (item.activityType === "mcq" && lesson.mcqActivity) {
          builtSteps.push(...mcqStepsFromLesson(lesson.mcqActivity, lName));
        } else if (item.activityType === "canvas" && lesson.canvasActivity) {
          builtSteps.push(canvasStepFromLesson(lesson.canvasActivity, lName));
        } else if (item.activityType === "quantus" && lesson.quantusActivity) {
          builtSteps.push(quantusStepFromLesson(lesson.quantusActivity, lName));
        }
      }
      ...
      setLoadError("Failed to load test data.");
    } finally {
      setLoading(false);
    }
  }, [testId, token]);
  ...
  if (loadError)
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#F0EDE7] gap-4">
        <p className="font-semibold text-zinc-500">{loadError}</p>
        <button
          onClick={() => router.push(`/admin/professions/${professionId}/tests`)}
          className="text-[#01696F] underline text-sm"
        >
          Back to Tests
        </button>
      </div>
    ...
    <header className="flex items-center gap-3 px-4 py-3 border-b border-zinc-200 bg-white shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-extrabold text-zinc-800 truncate">
          {testData?.name || "Skill Test"}
        </p>
        <div className="flex items-center gap-1.5 mt-0.5">
          {testData?.isPublished ? (
            <span className="flex items-center gap-1 text-[9px] font-black uppercase text-emerald-600">
              <Lock size={9} /> Published
            </span>
          ) : (
            <span className="text-[9px] font-black uppercase text-amber-600">Draft</span>
          )}
          {testData?.topic?.name && (
            <span className="text-[9px] text-zinc-400 font-semibold">· {testData.topic.name}</span>
          )}
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-[10px] font-bold text-zinc-400 bg-zinc-50 border border-zinc-200 px-3 py-1.5 rounded-xl">
    ...
    return (
      <div className="flex flex-col h-screen overflow-hidden bg-[#F8F8F7]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-zinc-600 p-8 text-center">
          <BookOpen size={36} className="text-[#01696F] opacity-40" />
          <p className="font-extrabold text-lg text-zinc-800">No activities to preview</p>
          <p className="text-sm text-zinc-500 max-w-sm">
            {testData?.isPublished
              ? "This published test has no activities."
              : "Publish the test first to preview its activities."}
          </p>
          <button
            onClick={() => router.push(`/admin/professions/${professionId}/tests`)}
              ...
              <div className="bg-[#FAF7F2] shadow-[0_2px_4px_0_#0000001F_inset] border border-[#F0EDE7] rounded-2xl p-3 flex flex-col gap-3 flex-1 overflow-y-auto">
                <div className="flex items-start justify-between gap-2 border-b border-zinc-200/50 pb-2">
                  <div className="min-w-0">
                    <h3 className="font-extrabold text-zinc-900 text-xs leading-tight tracking-tight truncate">
                      {testData?.name || "Skill Test"}
                    </h3>
                    {step?.lessonName && (
                      <p className="text-[9px] text-zinc-400 font-semibold mt-0.5 truncate">
                        From: {step.lessonName}
                      </p>
                    )}
                  </div>
                  {step && (
            ...
            {step?.stepType === "quantus" && (
              <div className="absolute inset-0 flex flex-col">
                <ExcelGrid
                  ...
                  isValidated={false}
                  feedback={{}}
                  sheetTabName={step.lessonName || "Sheet"}
                  showToolbar
        ...
        {!rightOpen && (
          ...
          >
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              <Trophy
                size={14}
                className="text-[#01696F] group-hover:scale-110 transition-transform duration-200"
              />
            </div>
          </button>
        )}
